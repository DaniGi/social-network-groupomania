...
// simple loader to use when waiting backend response
export default function Loader({ iconNumber }) {
  const icons = [];

  for (let i = 0; i < iconNumber; i += 1) {
    icons.push(
      <div key={i} className={`loader-icon loader-icon--${i + 1}`}>
  ...
  return (
    <div className="loader-container d-flex justify-content-center align-items-center">{icons}</div>
  );
}
